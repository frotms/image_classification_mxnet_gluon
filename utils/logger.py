...
class ExampleLogger:
    ...
    def write(self):
        ...
        :return: 
        """
        _info = 'epoch: %d, lr: %f, eval_train: %f, eval_validate: %f, train_loss: %f, validate_loss: %f, gpu_index: %s, net: %s, save: %s' % (
        self.log_info['epoch'],self.log_info['lr'], self.log_info['train_acc'], self.log_info['validate_acc'],
        self.log_info['train_loss'], self.log_info['validate_loss'],
...
class DefinedPrinter:
    ...
    def epoch_case_print(self, epoch, eval_train, eval_validate, loss_train, loss_validate, fitTime):
        ...
        log = "\nEpoch \033[1;36m" + str(epoch) + "\033[0m, " + \
              ...
              "\n\r" + \
              "Training loss \033[1;32m" + "{:.5f}".format(loss_train) + "\033[0m, " + \
              "Validating loss \033[1;32m" + "{:.5f}".format(loss_validate) + "\033[0m, " + \
              "epoch time " + str(fitTime) + ' s' + '\n'
        print('\n\r', log, '\n\r')
